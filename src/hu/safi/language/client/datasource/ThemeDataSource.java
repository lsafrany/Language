...
public class ThemeDataSource extends GwtRpcDataSource {
	...
	public ThemeDataSource() {
		...
		field = new DataSourceTextField(ClientConstants.THEME_KEY,
				ClientLabels.THEME_KEY);
		field.setPrimaryKey(true);
		field.setHidden(true);
		addField(field);

		field = new DataSourceTextField(ClientConstants.THEME_NAME,
				ClientLabels.THEME_NAME);
		...
		languageService.themes(new AsyncCallback<List<ThemeSer>>() {
			...
			public void onSuccess(List<ThemeSer> result) {
				...
				if ((result.size() > 0) && (result.get(0).getError() != null))
					response.setAttribute(ClientConstants.ERROR, result.get(0)
							.getError());
